...
export class Lineage {
    ...
    private readonly imageTeaser = ".image-text-teaser"
    ...
    async validateImageTeaser_Image(imageText:string){
        cy.get(this.imageTeaser).find(`.cmp-image__image[title*='${imageText}']`)
        .should('be.visible')
        cy.log(`Image with text ${imageText} is visible`)
    }
    ...
    async validateProfileTitle(titleText:string){
        cy.get(this.profile).find('h2').should('be.visible').should('have.text', titleText)
        cy.log(`The profile heading is visible with text ${titleText} on lineage page`)
    }
    ...
    async validateProfileImageHeading(headingText:string[]){
        ...
        .each((elem, index) =>{
            ...
            cy.log(`Profile image heading with text ${headingText[index]} is available on lineage page`)
        })
    }

    async validateFullWidthImageTeaserImageAtIndex(imageSrc:string, index:number){
        cy.get(this.fullWidthImageTeaser, {timeout:10000}).should('exist').eq(index-1).scrollIntoView()
        .find(`div>img[src*='${imageSrc}']`, {timeout:20000})
        .should('be.visible')
        cy.log(`Image with src text contains ${imageSrc} is visible under full width image teaser`)
    }

    async validateFullWidthImageTeaserTitle(titleText:string){
        cy.get(this.fullWidthImageTeaser).find(`h2`)
        .filter((_, el) => Cypress.$(el).text().trim() === titleText)
        .should('have.length', 1) 
        cy.log(`Image full width teaser with title text ${titleText} is visible`)
    }
